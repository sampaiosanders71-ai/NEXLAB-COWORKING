(function(){
    ...
    try {
      ...
      if (isSupabase) {
        if (response.status === 0 || response.status === 502 || response.status === 503 || response.status === 504 || response.status >= 500) {
          if (!target.includes('/rpc/nexlab_record_client_error_v26_7')) {
  ...
  const OBSERVABILITY_VERSION = '26.7';
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v26.7';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v26.7';
  const OBSERVABILITY_RPC = 'nexlab_record_client_error_v26_7';
    ...
    try {
      ...
      while (observabilityQueue.length && sentThisRun < 5) {
        ...
        if (
          ...
          observabilityState.lastError = 'Migration v26.7 não instalada.';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v26.7', JSON.stringify(performanceState));
    } catch {}
