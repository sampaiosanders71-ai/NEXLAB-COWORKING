(function(){
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v26.31.16';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v26.31.16';
  ...
  window.addEventListener('nexlab:react-recoverable-error', (event) => {
    ...
    const serious = /removeChild|Maximum call stack|recursion/i.test(`${detail.message || ''} ${detail.stack || ''}`);
    observabilityQueueEvent({
      ...
      stack: detail.stack || '',
      module: detail.module || document.body?.dataset?.nexlabPage || '',
      metadata: {
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v26.31.16', JSON.stringify(performanceState));
    } catch {}
