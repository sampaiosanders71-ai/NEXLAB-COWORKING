(function(){
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.22';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.22';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.22';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.22';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.22';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.22', JSON.stringify(performanceState));
    } catch {}
