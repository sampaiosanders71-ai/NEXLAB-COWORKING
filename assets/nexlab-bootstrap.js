(function(){
  ...
  const APP_VERSION = '26.30.31';
  const APP_RELEASE = 'R55.6.19';
  const APP_REVISION = 'permission-password-security-r55619';
  window.__NEXLAB_RELEASE__ = Object.freeze({version:APP_VERSION,release:APP_RELEASE,revision:APP_REVISION});
  ...
  const readSignalIds = new WeakMap();
  let readSignalSequence = 0;
  ...
  function pendingReadKey(key, signal){
    if (!signal || (typeof signal !== 'object' && typeof signal !== 'function')) {
      return `${key}|signal:none`;
    }
    let signalId = readSignalIds.get(signal);
    if (!signalId) {
      signalId = ++readSignalSequence;
      readSignalIds.set(signal, signalId);
    }
  ...
  if (nativeFetch) {
    window.fetch = async function(input, init){
      ...
      const signal = signalFrom(input, init);
      const pendingKey = pendingReadKey(key, signal);
      const now = Date.now();
      pruneReadCache(now);
      ...
      const pending = pendingReads.get(pendingKey);
      if (pending) {
        const sharedResult = await pending;
        if (!sharedResult.ok) throw sharedResult.error;
        return sharedResult.response.clone();
      }
      ...
      pendingReads.set(
        pendingKey,
        request.then(
          (response) => ({ ok: true, response: response.clone() }),
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v26.30.31';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v26.30.31';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v26.30.31', JSON.stringify(performanceState));
    } catch {}
