(function(){
  ...
  function v267CreateProductionPanel(){
    ...
    panel.innerHTML = `
      ...
      <div class="nexlab-v267-production-content" role="status" aria-live="polite">
        <p>Execute a migration v26.7 para ativar o monitoramento centralizado.</p>
      </div>
    `;
  ...
  function v267RenderProduction(panel, summary){
    ...
    status.append(migration, latest, delivery);
  ...
  async function v267LoadProductionPanel(panel, force = false){
    ...
    try {
      ...
      const response = await fetch(
        `${String(config.supabaseUrl).replace(/\/+$/, '')}/rest/v1/rpc/nexlab_get_observability_summary_v26_7`,
        {
          method: 'POST',
          headers: {
            apikey: config.supabaseAnonKey,
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 404 ||
          /PGRST202|nexlab_get_observability_summary_v26_7/i.test(body)
        ) {
          throw new Error('Execute o SQL da v26.7 para ativar o monitoramento.');
        }
