(function(){
  ...
  if(window[GLOBAL_KEY]?.version==='26.29.3') return;
  ...
  function mount(){
    ...
    if(!legacy){unmount();return;}
    state.active=true;state.legacy=legacy;legacy.dataset.nexlabActivitiesLegacy='true';legacy.hidden=true;legacy.setAttribute('aria-hidden','true');
    if(!state.root||!state.root.isConnected){
      ...
      legacy.parentNode.insertBefore(state.root,legacy);bind();syncControlValues();updateNetwork();renderLoading(true);load(true);setupRealtime();
    }else if(state.root.parentNode!==legacy.parentNode){legacy.parentNode.insertBefore(state.root,legacy);}
    state.root.hidden=false;
  }
  function unmount(){
    state.active=false;if(state.root)state.root.hidden=true;
    if(state.legacy?.isConnected){state.legacy.hidden=false;state.legacy.removeAttribute('aria-hidden');delete state.legacy.dataset.nexlabActivitiesLegacy;}
    state.legacy=null;
  }
  function bind(){
  ...
  async function load(first=false,options={}){
    ...
    }catch(error){console.error('Falha ao carregar a Central de Atividades:',error);renderError(error?.message||'Não foi possível carregar os registros.');updateLive(navigator.onLine?'polling':'offline');}
    finally{
  ...
  function setupRealtime(){
    if(state.channel||state.pollTimer)return;
    const connect=()=>{
      const client=window.__NEXLAB_SUPABASE__;
      if(!client?.channel){state.pollTimer=setInterval(()=>{if(state.active&&document.visibilityState==='visible'&&navigator.onLine)load();},60000);updateLive(navigator.onLine?'polling':'offline');return;}
      state.channel=client.channel('nexlab-activities-v26280')
        ...
        .subscribe(status=>{
          ...
          if(['CHANNEL_ERROR','TIMED_OUT','CLOSED'].includes(status)){updateLive(navigator.onLine?'polling':'offline');if(!state.pollTimer)state.pollTimer=setInterval(()=>state.active&&navigator.onLine&&load(),60000);}
        });
    };
    if(window.__NEXLAB_SUPABASE__)connect();else setTimeout(connect,1500);
  }

  readPreferences();
  state.observer=new MutationObserver(()=>{clearTimeout(state.routeTimer);state.routeTimer=setTimeout(()=>{if(isLogsScreen())mount();else unmount();},80);});
  state.observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style','hidden','aria-current','data-nexlab-page']});
  document.addEventListener('visibilitychange',()=>{if(state.active&&document.visibilityState==='visible'){if(state.pendingRealtime)renderRealtimeBanner();else if(navigator.onLine)scheduleRefresh();}});
  window.addEventListener('online',()=>{updateNetwork();if(state.active)scheduleRefresh();});
  window.addEventListener('offline',updateNetwork);
  setInterval(()=>{if(isLogsScreen())mount();else unmount();},1200);

  window[GLOBAL_KEY]={version:VERSION,state,refresh:()=>load(),destroy(){state.observer?.disconnect();clearInterval(state.pollTimer);state.channel?.unsubscribe?.();state.root?.remove();unmount();}};
})();
