(function(){
  'use strict';
  if(window.__NEXLAB_VAPID_ROTATION_R55613__) return;
  window.__NEXLAB_VAPID_ROTATION_R55613__={version:'26.30.26',status:'idle'};
  ...
  async function rotate(){
    const state=window.__NEXLAB_VAPID_ROTATION_R55613__;state.status='checking';
    ...
    localStorage.setItem(MARKER,'26.30.26');
    state.status=same?'refreshed':'rotated';state.endpoint=subscription.endpoint;state.completedAt=new Date().toISOString();
    window.dispatchEvent(new CustomEvent('nexlab:vapid-rotated',{detail:{status:state.status,version:'26.30.26'}}));
  }
  const start=()=>rotate().catch(error=>{window.__NEXLAB_VAPID_ROTATION_R55613__.status='error';window.__NEXLAB_VAPID_ROTATION_R55613__.error=String(error?.message||error)});
  if(document.readyState==='complete')setTimeout(start,1200);else window.addEventListener('load',()=>setTimeout(start,1200),{once:true});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&localStorage.getItem(MARKER)!=='26.30.26')start()});
})();
