(function(){
  ...
  const RELEASE='R54.3';
  const KEY='__NEXLAB_MURAL_R543__';
  ...
  function findLegacy(){
    ...
    return [...document.querySelectorAll('.module-shell')].find(element=>{
      if(element.closest('[data-nexlab-mural-r543]')||!visible(element))return false;
  ...
  function shell(){
    return `<section class="nexlab-mural-r543" data-nexlab-mural-r543="true" aria-labelledby="mural-r543-title">
      <header class="mural-r543-hero">
        <div><span>Comunicação interna</span><h1 id="mural-r543-title">Mural de comunicados</h1><p>Publicações com rastreabilidade, mídia privada e proteção contra envios duplicados.</p></div>
        <div class="mural-r543-status"><div class="mural-r543-sync" data-role="sync" data-state="connecting" role="status" aria-live="polite"><span class="mural-r543-sync-dot" aria-hidden="true"></span><span data-role="sync-label">Conectando ao tempo real…</span></div><div class="mural-r543-version">${RELEASE} · v${VERSION}</div></div>
      </header>
      <div class="mural-r543-alert" data-role="alert" hidden></div>
      <div class="mural-r543-toolbar" data-role="toolbar"></div>
      <div class="mural-r543-editor" data-role="editor" hidden></div>
      <div class="mural-r543-feed" data-role="feed" aria-live="polite"></div>
    </section>`;
  }
  function mount(){
  ...
  function renderToolbar(){
    ...
    toolbar.innerHTML=`<div class="mural-r543-tabs" role="tablist" aria-label="Situação das publicações">
    ...
    </div><div class="mural-r543-actions">
  ...
  function connectRealtime(){
    ...
    state.channel=supabase.channel(`nexlab-mural-r543-${uuid()}`)
  ...
  function render(){
    ...
    if(!rows.length){
      feed.innerHTML=`<div class="mural-r543-empty"><strong>${state.mode==='archived'?'Nenhuma publicação arquivada':'O Mural ainda não possui publicações'}</strong><span>${state.mode==='archived'?'As publicações arquivadas poderão ser restauradas aqui.':'Use “Nova publicação” para criar o primeiro comunicado.'}</span></div>`;
      return;
    }
    feed.innerHTML=rows.map(post=>{
      ...
      return `<article class="mural-r543-card ${post.fixado?'pinned':''} ${archived?'archived':''}">
        ${post._cover_signed_url?`<figure class="mural-r543-cover"><img src="${esc(post._cover_signed_url)}" alt="Capa da publicação ${esc(post.titulo||'')}" loading="lazy"></figure>`:''}
        <div class="mural-r543-card-body">
          <div class="mural-r543-card-top"><div>${post.fixado?'<span class="mural-r543-badge">Fixado</span>':''}${archived?'<span class="mural-r543-badge muted">Arquivado</span>':''}</div><div class="mural-r543-card-actions">${controls.join('')}</div></div>
          ...
          ${archived?`<div class="mural-r543-archive-info"><strong>Motivo:</strong> ${esc(post.archive_reason||'Não informado')}<br><small>Arquivada em ${fmt(post.archived_at)}${post.archived_by_name?` por ${esc(post.archived_by_name)}`:''}</small></div>`:''}
        </div>
      </article>`;
    }).join('');
  }
  async function load(options={}){
    ...
    state.loading=true;
    if(!silent)state.root.querySelector('[data-role="feed"]').innerHTML='<div class="mural-r543-loading">Carregando publicações…</div>';
    try{
      ...
      if(!silent||!state.data)state.root.querySelector('[data-role="feed"]').innerHTML=`<div class="mural-r543-error"><strong>Não foi possível carregar o Mural.</strong><span>${esc(error.message||error)}</span><button data-action="refresh">Tentar novamente</button></div>`;
    }finally{
      state.loading=false;
  ...
  function coverPreview(post){
    ...
    return `<div class="mural-r543-media-preview" data-role="media-preview">${source?`<img src="${esc(source)}" alt="Prévia da capa"><button type="button" data-action="remove-cover">Remover capa</button>`:'<div><strong>Sem imagem de capa</strong><span>JPG, PNG ou WebP, até 5 MB. O arquivo será armazenado de forma privada.</span></div>'}</div>`;
  }
  function openEditor(post=null){
    ...
    box.innerHTML=`<form data-form="post" novalidate>
      <div class="mural-r543-editor-head"><div><strong>${post?'Editar publicação':'Nova publicação'}</strong><span>${post?'A autoria original será preservada e a edição ficará registrada.':'Envios repetidos serão detectados automaticamente.'}</span></div><button type="button" data-action="cancel" aria-label="Fechar editor">×</button></div>
      <div class="mural-r543-fields">
        ...
        <label class="mural-r543-file-label">Imagem de capa<input type="file" name="cover_file" accept="image/jpeg,image/png,image/webp"><small>O arquivo não fica público e é exibido por URL temporária assinada.</small></label>
        ${coverPreview(post)}
        ${canModerate?`<label class="mural-r543-check"><input type="checkbox" name="pinned" ${post?.fixado?'checked':''}> Fixar esta publicação</label>`:''}
      </div>
      <div class="mural-r543-editor-actions"><button type="button" data-action="cancel">Cancelar</button><button class="primary" type="submit" data-role="submit">${post?'Salvar alterações':'Publicar'}</button></div>
    </form>`;
