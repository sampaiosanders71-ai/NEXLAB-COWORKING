(function(){
  ...
  // R55.4.5: recuperação não bloqueante após o loop do atualizador anterior.
  try {
    document.documentElement.removeAttribute('data-nexlab-busy');
    document.documentElement.removeAttribute('aria-busy');
    for (const key of Object.keys(sessionStorage)) {
      if (key.startsWith('nexlab:update:')) sessionStorage.removeItem(key);
    }
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith('nexlab:update:')) localStorage.removeItem(key);
    }
    const clearUpdateUi = () => {
    ...
    clearUpdateUi();
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', clearUpdateUi, { once: true });
    window.setTimeout(clearUpdateUi, 500);
    window.setTimeout(clearUpdateUi, 2500);
  } catch {}
  ...
  // R55.4.5: preserva o bloqueio imediato de mutações para impedir cliques duplos antes do React atualizar a interface.
